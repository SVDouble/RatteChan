...
from ratte.config import Config, ExperimentConfig
from ratte.contours import Contour
from ratte.models import SensorData, Stats
from ratte.utils import combine_mean_std, format_mean_std
...
class Monitor:
    ...
    def plot_defl_profile(self, defl_model):
        ...
        f = plt.figure()
        plt.scatter(points[:, 0] * 1e3, points[:, 1] * 1e3, c=deflections, cmap="viridis")
        plt.xlabel("X, mm")
        plt.ylabel("Y, mm")
        plt.title("Deflection Profile")
        plt.colorbar(label="Deflection, rad")
        plt.show()
