...
class TipEstimator:
    def __init__(self, initial_state: WorldState):
        ...
        self.tip_s_filter.x = self.defl_model.get_position(
            initial_state.wr0_yaw_s
        ).reshape(-1, 1)
        self.tip_s_deque = deque(maxlen=20)

    def update_wr0_yaw_s(self, state: WorldState) -> None:
        # get local tip position from deflection
        tip = self.defl_model.get_position(state.wr0_yaw_s)
    ...
    def get_w(self, state: WorldState) -> np.ndarray:
        tip_s = self.tip_s_filter.x.flatten()
        return rotate_ccw(tip_s, state.body_yaw_w) + state.body_r_w
